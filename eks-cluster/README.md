...
| <a name="input_nodegroup"></a> [nodegroup](#input\_nodegroup) | n/a | <pre>list(object({<br/>    instance_types  = list(string)<br/>    capacity_type   = optional(string, "SPOT") # or ON_DEMAND<br/>    auto_scale_options = list(object({<br/>      min     = number<br/>      max     = number<br/>      desired = number<br/>    }))<br/>    labels = optional(map(string), {})<br/>  }))</pre> | n/a | yes |
